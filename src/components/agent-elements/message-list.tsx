...
export const MessageList = memo(function MessageList({
  ...
  const showPlanning = useMemo(() => {
    // Only show idle thinking before the first assistant content/tools appear.
    // Never after the model already streamed text or ran tools.
    if (!isStreaming) return false;
    const lastMessage = normalizedMessages[normalizedMessages.length - 1];
    if (!lastMessage) return false;
    if (getLastAssistantHasContent(normalizedMessages)) return false;
    ...
    // assistant exists but empty and no tool parts yet
