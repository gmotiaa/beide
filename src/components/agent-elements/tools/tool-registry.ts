...
function getDisplayPath(filePath: string): string {
  ...
  // Windows absolute → show last 2–3 segments
...
function pickFilePath(input: Record<string, unknown> | undefined): string {
  if (!input) return "";
  const v =
    input.file_path ?? input.filePath ?? input.path ?? input.filename ?? input._basename;
  return typeof v === "string" ? v : "";
}

function pickCommand(input: Record<string, unknown> | undefined): string {
  if (!input) return "";
  const v = input.command ?? input.cmd ?? input.shell;
  return typeof v === "string" ? v : "";
}
...
export const toolRegistry: Record<string, ToolMeta> = {
  ...
  "tool-Read": {
    ...
    subtitle: (part) => {
      ...
      if (!filePath) return "";
      const display = getDisplayPath(filePath);
      const base = display.replace(/\\/g, "/").split("/").pop() || display;
      return base;
    },
    variant: "simple",
  },
  "tool-GitStatus": {
    ...
    subtitle: (part) => {
      ...
      if (branch && dirty === true) return `${branch} · dirty`;
      if (branch && dirty === false) return `${branch} · clean`;
  ...
  "tool-ProjectInfo": {
    ...
    subtitle: (part) => {
      ...
      return typeof name === "string" ? name : "package.json";
    },
    variant: "simple",
  },
  "tool-Edit": {
    ...
    title: (part) => {
      ...
      const display = getDisplayPath(filePath);
      return display.replace(/\\/g, "/").split("/").pop() || "Edit";
    },
    subtitle: (part) => {
      const isPending =
        part.state !== "output-available" && part.state !== "output-error";
  ...
  "tool-Write": {
    ...
    subtitle: (part) => {
      ...
      if (!filePath) return "";
      const display = getDisplayPath(filePath);
      return display.replace(/\\/g, "/").split("/").pop() || display;
    },
    variant: "simple",
  },
  "tool-Bash": {
    ...
    subtitle: (part) => {
      const command = pickCommand(part.input);
